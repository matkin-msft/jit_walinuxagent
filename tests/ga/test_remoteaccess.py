...
class TestRemoteAccess(AgentTestCase):
    ...
    def test_update_remote_access_conf_no_remote_access(self, _):
        ...
        self.assertNotEquals(None, protocol.client.RemoteAccess)
        self.assertEquals(0, len(protocol.client.RemoteAccess.Users))
    ...
    def test_update_remote_access_conf_remote_access(self, _1, _2, _3):
        ...
        protocol.client.update_remote_access_conf(goal_state)
        self.assertNotEquals(None, protocol.client.RemoteAccess)
        self.assertEquals(1, len(protocol.client.RemoteAccess.Users))
        self.assertEquals('testAccount', protocol.client.RemoteAccess.Users[0].Name)
        self.assertEquals('encryptedPasswordString', protocol.client.RemoteAccess.Users[0].EncryptedPassword)
        self.assertEquals(2, len(protocol.client.RemoteAccess.Users[0].Groups))
        self.assertEquals('Administrators', protocol.client.RemoteAccess.Users[0].Groups[0])
        self.assertEquals('RemoteDesktopUsers', protocol.client.RemoteAccess.Users[0].Groups[1])
